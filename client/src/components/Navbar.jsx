...
export default function Navbar() {
  ...
  const {userAuthenticated, setTriggerUserConnexion } = useContext(AuthContext);
  ...
  useEffect(
    () => {
      ...
      const newListOfPosts = listOfPosts.filter((post) =>
        `${post.title} ${post.description} ${post.User.email} ${post.User.username}`.includes(searchTerm)
      );
  ...
  return (
    ...
    <nav style={{boxShadow:"0px 0px 16px -7px #0d0d0d"}}className={`h-[60px] w-full z-20 fixed ${darkMode ? 'bg-[#23252b]' : 'bg-[#fcf6eb]'}`} 
    >
      <ul className='h-full w-full flex items-center justify-between px-10'>
        ...
        <li>
          <input type="text"
            id="search"
            name="search"
  onChange={(value) => onChange(value)} placeholder="Rechercher" className={`searchInput ${darkMode ? '  text-[#919197]' :'  text-[#434346]' } border-[#424040] border-2 `}  />
        </li>
        <li className="w-[240px] flex justify-end">
          ...
          {!userAuthenticated ? <div className=" font-bold flex">
            ...
            : <div className='flex'><NavLink to='profile' className="mr-3 flex items-center justify-center text-[#b15f5f] border hover:bg-[#8b4545] hover:text-white ease-in-out duration-200 border-[#b15f5f] w-10 h-10 rounded-[20px]"><FaUser /></NavLink> <button onClick={handleDeleteCookie} className="py-1.5 px-2.5 text-white ease-in-out duration-200 rounded w-30 bg-[#b15f5f] flex items-center hover:bg-[#8b4545]"><FontAwesomeIcon className="mr-2" icon="user" />Déconnexion</button>
            </div>}
        </li>
