...
const Profile = () => {
    ...
    if (selectedFile) {
      ...
      axios
        ...
        .then((res) => {
          ...
          console.log(user.id);
          axios
            .put(`http://${API_HOST}:${API_PORT}/users/${user.id}`, {
              id_banner: id_banner,
            })
            .then((res) => {
              console.log(res);
  ...
  const handleUserInfoOk = () => {
    axios
      .put(`http://${API_HOST}:${API_PORT}/users/${user.id}`, { bio, username })
      .then((response) => {
  ...
  return isLoading ? (
    ...
    <div className="flex relative top-[60px] z-10 flex-col w-full items-center">
      ...
      <div
        className="h-[500px] w-full flex flex-col items-center  justify-end"
        style={{
          backgroundImage: user?.banner
            ? `url(http://${API_HOST}:${API_PORT}/uploads/${user.banner.name})`
      ...
      >
        <button
          className="w-[100px] text-sm h-[35px] border-2 relative top-[40px] transition ease-in-out duration-200 border-[#f1f1f18c] rounded-[20px] text-[#f1f1f18c] hover:border-white hover:text-white"
          onClick={showModal}
        >
          Modifier
        </button>
        ...
        {/* profile pic */}
        <div
          className="h-36 w-36 relative top-[72px] cursor-pointer scale-100	transition ease-in-out duration-200 hover:scale-105 rounded-[50px]"
          style={{
        ...
        <CustomModal
          ...
          isOpen={isModalPpOpen}
          onOk={handlePpOk}
          onCancel={handlePpCancel}
          darkMode={darkMode}
        >
      ...
      <div className="flex flex-col items-center relative top-[72px] w-full">
        <div>
          <p className=" w-full font-semibold text-2xl mb-2 mt-1" style={{}}>
            {user.username}{" "}
          </p>
        </div>
        <hr className=" border-[#454649] w-5/12"></hr>
        <p className="w-5/12 text-center my-3">
          {!user.bio ? "ajouter une description" : user.bio}
        </p>
        <hr className="border-[#454649] w-5/12"></hr>
        <button
          onClick={showModalUserInfos}
          className="w-[90px] mt-2 text-sm h-[35px] border-2 transition ease-in-out duration-200 border-[#f1f1f18c] rounded-[20px] text-[#f1f1f18c] hover:border-white hover:text-white"
        >
          Editer
        </button>
        ...
        <div className="flex flex-col items-center w-full">
          <div className="w-9/12 flex-col flex">
            ...
            <textarea
              maxlength="254"
              id="bio"
              className="outline-none bg-transparent border border-[#924a4a] p-2"
              value={bio}
          ...
          <div className="w-9/12 mt-2 flex-col flex">
            ...
            <input
              type="text"
              id="username "
              className="outline-none border border-[#924a4a] bg-transparent p-2"
              value={username}
